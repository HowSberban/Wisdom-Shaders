...
float get_exposure() {
	...
	// Sample multiple points across the screen to get average luminance
	// This prevents flickering from localized highlights like water reflections
	float total_luma = 0.0;
	int sample_count = 0;
	
	// Sample in a cross pattern around the center
	vec2 samples[9] = vec2[](
		vec2(0.5, 0.5),      // Center
		vec2(0.3, 0.5),      // Left
		vec2(0.7, 0.5),      // Right
		vec2(0.5, 0.3),      // Top
		vec2(0.5, 0.7),      // Bottom
		vec2(0.35, 0.35),    // Top-left
		vec2(0.65, 0.35),    // Top-right
		vec2(0.35, 0.65),    // Bottom-left
		vec2(0.65, 0.65)     // Bottom-right
	);
	
	for (int i = 0; i < 9; i++) {
		#ifdef BLOOM
		vec3 color = texture2D(gcolor, samples[i]).rgb;
		#else
		vec3 color = texture2D(composite, samples[i]).rgb;
		#endif
		total_luma += luma(color);
		sample_count++;
	}
