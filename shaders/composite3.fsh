...
void main() {
	...
	if (mask.is_valid || isEyeInWater) {
		...
		// Apply atmosphere with indoor/outdoor check (like Bliss shader)
		if (!isEyeInWater) {
			float sky_access = clamp(eyeBrightnessSmooth.y / 240.0, 0.0, 1.0);
			calc_fog_height (land, 0.0, 512.0 * (1.0 - cloud_coverage), color, atmosphere * (0.7 * lit_strength + 0.3) * sky_access);
		}
		#endif
	}
